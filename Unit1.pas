...
begin
  ...
  radek := GetLineIndexOfText(MemoStranka, 'ParcelnÌ');
  Parcela := GetTextAfterPrefixInLine(MemoStranka, 'ParcelnÌ ËÌslo:	', radek);
  StringGrid1.Cells[7, row] := Parcela;

  radek := GetLineIndexOfText(MemoStranka, '»Ìslo LV');
  LV := GetTextAfterPrefixInLine(MemoStranka, '»Ìslo LV:	', radek);
  StringGrid1.Cells[9, row] := LV;

  radek := GetLineIndexOfText(MemoStranka, 'V˝mÏra');
  Vymera := GetTextAfterPrefixInLine(MemoStranka, 'V˝mÏra [m2]:	', radek);
  ...
  radek := GetLineIndexOfText(MemoStranka, 'Zp˘sob vyuûitÌ');
  Vyuziti := GetTextAfterPrefixInLine(MemoStranka, 'Zp˘sob vyuûitÌ:	', radek);
  ...
  if (Copy(MemoStranka.Lines[radek + 1], 1, 2)) = 'Ne' then
    ...
    if (Copy(MemoStranka.Lines[radek + 1], 1, 2)) = 'N·' then
      begin
        ochrana := '';
        StringGrid1.Cells[17, row] := ochrana;
      end
  ...
  radek := GetLineIndexOfText(MemoStranka, 'OmezenÌ vlastnickÈho');
  ...
  radek := GetLineIndexOfText(MemoStranka, 'VlastnickÈ pr·vo');
  if (Copy(MemoStranka.Lines[radek + 2], 1, 11)) = 'P¯Ìsluönost' then
  begin
    try
      begin
  ...
  if parcela_radek = (MemoParcely.Lines.Count) then
    begin
      ...
      Button15.Font.Style := Font.Style - [TFontStyle.fsBold];
      Showmessage('Vöechny parcely byly vysp·ny, m˘ûe b˝t proveden export do Excelu.');
    end
    else
    begin
...
begin
  VyplnitDoExcelu(StringGrid1, ExtractFilePath(Application.ExeName) + 'Seznam dotËen˝ch vlastnÌk˘.xlsx');
end;
...
begin
  ...
  Stringgrid1.Cells[13, 0] := 'V˝mÏra';
  Stringgrid1.Cells[14, 0] := 'Druh pozemku';
  Stringgrid1.Cells[15, 0] := 'Zp˘sob vyuûitÌ';
  Stringgrid1.Cells[16, 0] := 'Platnost';
  Stringgrid1.Cells[17, 0] := 'Zp˘sob ochrany';
  Stringgrid1.Cells[18, 0] := 'OmezenÌ';
  Stringgrid1.Cells[19, 0] := 'JmÈno';
  Stringgrid1.Cells[20, 0] := 'PodÌl';
